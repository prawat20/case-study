...
| Active users / DAU | Vanity. PMs may open the tool daily without making decisions. |
| Decisions logged per quarter | Closer, but a count metric inflates with chaos — not what we want to optimize. |
...
| Stakeholder alignment latency (hours from conflict-flagged to resolved) | Is the coordination loop compressing? | The 2025 SOPM finding: AI saves time, coordination eats it. This catches when our own product makes that worse. |
| Priority churn rate (re-sequences per quarter) | Is continuous synthesis creating chaos? | Continuous re-prio sounds great until stakeholders lose trust in the plan. Churn rate is the safety net. |
...
Each event includes a rationale grounded in *what product decision it informs*. Events that don't drive a decision shouldn't ship.
...
**Why this event:** It powers the North Star (`time_to_decide_ms`) and the most important supporting metric (override rate). Slicing by theme/signal-type tells us where the AI is consistently wrong. Slicing by `framework_was_overridden` tells us whether AI's framework picker is trusted.
...
**Why this event:** Our Dynamic Priority Engine claims to flag re-prioritization continuously. This event tells us whether the flags are *useful* (PMs act on them) or *noise* (PMs ignore them). If `was_actioned` rate is below 30%, the engine is generating noise — we tighten the threshold. The metric directly defends or kills our most ambitious capability claim.
...
**Why this event:** The audience render is one of two demo-winning surfaces. This event tells us *which* audience renders PMs actually use. If the Sales view is opened once a quarter and the Exec view is opened weekly, we invest copy quality in Exec and treat Sales as a stub. If no one ever toggles, the whole capability is over-engineered — we'd cut it. Critical product decision.

### Event 4: `framework_overridden`

**Fires when** the PM picks a framework different from the AI's default (the brief: "framework of user's choice").
...
**Why this event:** Closes the loop on the "framework of user's choice" requirement. Aggregated, this event tells us whether the AI's per-item framework picker is matched to PM intuition or fighting it. If `Strategic Bet` is consistently overridden to `RICE` for compliance items, the AI's framework heuristic needs adjustment. This is the *learning fuel* for the framework picker specifically.
...
- ❌ `feedback_submitted` — premature; we don't have a feedback surface yet.

If we had budget for a sixth event, it would be `decision_outcome_observed` — fires quarterly when the predicted outcome of a committed decision is compared to actuals. But that requires real ingestion to ship (which the brief explicitly skips), so it's deferred.
...
We need to understand how they actually plan today before we test our solution. *Don't lead.*
...
Now we're hunting for the painkiller. *Listen for the words they use.*
...
Concrete tool usage tells us the competitive landscape and the workarounds.
...
*Now* we test the wedge — without showing the product yet.
...
Five interviews of mid-stage B2B SaaS PMs is the minimum bar. If 3+ are validating across all three dimensions, we have a wedge. If fewer, we re-frame and re-interview.
