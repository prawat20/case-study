...
**Sound.** Five Web Audio chimes, gain tuned ~30% softer than a typical dark-context UI — cream needs quieter sound. Capture pluck (A5), three triage tones by action (G4/B4/D5), commit two-tone (D5+A5), snap three-note resolving chord. User-mutable.
...
**State.** No backend. State persists in client-side localStorage across seven stores (decisions, framework overrides, triage actions, captures, calendar assignments, calendar lock state, theme preference). Cmd+K → Reset nukes all seven.
...
- **Surface consolidation pays.** Inbox folded into Now (`/inbox` kept as a transparent redirect for deep-link compatibility) reduced top-nav from 5 → 4 items and "land → first triage card" from 2 clicks to 1. Anywhere a surface is just a container around content from another surface, fold.
...
- **localStorage is enough for the demo surface.** The brief explicitly assumes ingestion is solved upstream. Persisting state in seven scoped localStorage stores (each scoped by purpose, each independently resettable) covers every reviewer flow without inviting backend complexity that wasn't in scope.
